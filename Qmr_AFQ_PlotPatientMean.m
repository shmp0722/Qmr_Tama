...
%% bar plot checking oatient data
% loop over patient
...
%% bar plot checking control data
% loop over patient
for k = 1:sum(afq.sub_group)
    for v = 1:length(valname)
        figure; hold('on');
        ...
        for ii = 1:nfg
            % Get the values for the patient and compute the mean
            vals_p = cVals(ii).(upper(valname{v}))(k,:);
        ...
        subnames = afq.sub_dirs(afq.sub_group==0);
        [p, ~]= fileparts(subnames{k});
        [~,f]= fileparts(p);
...
AFQ_MakeFiberGroupMontage(afq,afq.fgnames,[])
